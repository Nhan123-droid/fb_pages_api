...
namespace Page_API.Services
{
    public class FacebookEventConsumerService : BackgroundService
    {
        ...
        private void ConsumeLoop(CancellationToken stoppingToken)
        {
            ...
            var topics = _options.Topic.Split(',').Select(t => t.Trim()).ToList();
            ...
            try
            {
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    if (!TryDeserialize(consumeResult.Message.Value, out var commandMessage))
                    {
                        _logger.LogWarning(
                    ...
                    try
                    {
                        ...
                        handler.HandleAsync(commandMessage!, stoppingToken).GetAwaiter().GetResult();
                    ...
                    catch (Exception ex)
                    {
                        _logger.LogError(
                            ex,
                            "Failed to process event at {TopicPartitionOffset}. CommandId={CommandId}",
                            consumeResult.TopicPartitionOffset,
        ...
        private bool TryDeserialize(string payload, out BackendCommandMessage? message)
        {
            message = null;
            try
            {
                message = JsonSerializer.Deserialize<BackendCommandMessage>(payload, _jsonOptions);
                if (message is null || string.IsNullOrWhiteSpace(message.CommandId))
                {
                    return false;
                }
