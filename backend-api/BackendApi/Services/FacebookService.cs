...
namespace Page_API.Services
{
    public class FacebookService : IFacebookService
    {
        ...
        public async Task<object?> ReplyToCommentAsync(string commentId, string message, CancellationToken cancellationToken = default)
        {
            var token = System.Net.WebUtility.UrlEncode(_options.PageAccessToken);
            var response = await _httpClient.PostAsJsonAsync($"{commentId}/comments?access_token={token}", new { message }, cancellationToken);
            if (!response.IsSuccessStatusCode)
            {
                await ThrowFacebookApiException(response);
            }
